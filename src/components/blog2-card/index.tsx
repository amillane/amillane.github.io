...
const BlogCard2 = ({
  ...
  const renderSkeleton = () => {
    ...
    for (let index = 0; index < externalProjects.length; index++) {
      array.push(
        <div className="card shadow-lg compact bg-base-100 mb-0.5" key={index}>
  ...
  const renderExternalProjects = () => {
    return externalProjects.map((item, index) => (
      <a
        className="card shadow-lg compact bg-base-100 mb-0.5 cursor-pointer"
        key={index}
        href={item.link}
        onClick={(e) => {
          e.preventDefault();
            ...
            {item.tags && item.tags.length > 0 && (
              <div className="mt-2">
                {item.tags.map((tag, tagIndex) => (
                  <span key={tagIndex} className=""m-1 text-xs inline-flex items-center font-bold leading-sm px-3 py-1 badge-primary bg-opacity-90 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
